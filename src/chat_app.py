...
# # LLM client for the handoff service (setting up the handoff service)
        ...
        while True:
            ...
            try:
                ...
                if image_url:
                    ...
                    # Fallback: extract image URL from message text if user typed it inline
                    _url_match = re.search(r'(https?://\S+\.(?:jpg|jpeg|png|gif|webp|bmp|svg)(?:\?\S*)?)', user_message, re.IGNORECASE)
                    if not _url_match:
                        # Also try generic image hosting URLs (unsplash, imgur, etc.)
                        _url_match = re.search(r'(https?://(?:images\.unsplash\.com|i\.imgur\.com|imgur\.com)\S+)', user_message, re.IGNORECASE)
                    if _url_match:
                        persistent_image_url = _url_match.group(1)
                        image_url = persistent_image_url
                        logger.info("Image URL extracted from message text", extra={"url": persistent_image_url})
                        asyncio.create_task(pre_fetch_image_description(persistent_image_url, image_cache))
            ...
            # # Single-agent (actually, a GenAI model - AOAI gpt-5-mini) example
            # try:
            ...
            try:
                ...
                if image_data or products or persistent_image_url:
                    context_parts = []
                    if persistent_image_url:
                        context_parts.append(f"Image URL: {persistent_image_url}")
                    if image_data:
